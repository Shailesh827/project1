...
export default function BuyerReg()
{
    ...
    const init={
        ...
        phone:{ value: "", error: "", valid: false, touched: false },
    ...
    const validate = (nm, val) => {
      ...
      const sendData = (e) => {
        ...
        fetch("http://localhost:8080/regBuyer", reqOptions)
          .then((resp => {
            if(resp.ok){
        ...
        <form>
          ...
          <div className="mb-3">
            ...
            <div
              id="phoneHelp"
              className="form-text"
              style={{ display: (!info.phone.valid && info.phone.touched) ? "block" : "none" }}>
              {info.phone.error}
            </div>
          </div>
